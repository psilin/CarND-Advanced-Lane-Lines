    ...
    def run(self, left_fit, right_fit, check_value):
        if self.init is False:
            ...
            self.right = self.right * (1. -
                                       self.alpha) + right_fit * self.alpha
...
def calibrateCamera(path):
    ...
    (ret, mtx, dist, rvecs,
        tvecs) = cv2.calibrateCamera(objpoints, imgpoints, shape, None, None)
...
def apply_mask(img):
    ...
    out_reg_thres = ((YY < 450) | (YY < (XX*fit_left[0] + fit_left[1])) |
                     (YY < (XX*fit_right[0] + fit_right[1])) |
                     ((YY > (XX*fit_inner_left[0] + fit_inner_left[1])) &
                     (YY > (XX*fit_inner_right[0] + fit_inner_right[1]))))
    region_select[out_reg_thres] = 0  # [0, 0, 0]
...
def apply_color_spaces(img):
    ...
    # Stack each channel to view their individual
    # contributions in green and blue respectively.
    # This returns a stack of the two binary images,
    # whose components you can see as different colors.
    color_binary = np.dstack(
        (np.zeros_like(l_binary), l_binary, b_binary)) * 255
...
def get_curvature(binary_warped, left_fit, right_fit):
    ...
    right_curverad = ((1 +
                      (2*right_fit_cr[0]*y_eval*ym_per_pix +
                       right_fit_cr[1])**2)**1.5) / np.absolute(
                                                        2*right_fit_cr[0])
...
def window_search(binary_warped, low_pass):
    ...
    left_curve_rad, right_curve_rad = get_curvature(binary_warped,
                                                    left_fit, right_fit)
...
def make_pipeline(M, Minv, mtx, dist, low_pass):
    ...
    def pipeline(img):
        ...
        warped = cv2.warpPerspective(undistorted_masked,
        ...
        (lane_img, left_curve_rad, right_curve_rad,
            center_offset) = window_search(combined_binary, low_pass)
        unwarped = cv2.warpPerspective(lane_img,
                                       Minv, img_size, flags=cv2.INTER_LINEAR)
        unwarped = cv2.addWeighted(undistorted, 1., unwarped, 0.3, 0)
        cv2.putText(unwarped, 'Offset: {:.2f} m'.format(center_offset),
                    (40, 40), cv2.FONT_HERSHEY_SIMPLEX, 1., (255, 0, 0),
                    2, cv2.LINE_AA)
        cv2.putText(unwarped,
                    'Curve rad: {:.2f} km'.format((left_curve_rad +
                                                  right_curve_rad) / (2*1000)),
                    (40, 80), cv2.FONT_HERSHEY_SIMPLEX, 1., (255, 0, 0),
                    2, cv2.LINE_AA)
        return unwarped
